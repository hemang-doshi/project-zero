...
Updated: **2026-09-24 11:42 IST**. Start with [AGENTS.md](AGENTS.md) and [snapshot.md](snapshot.md). The approved redesign plans are implemented in the isolated Electron worktree; this handoff records verified behavior, Git publication constraints and remaining gates.

## Current source and recovery

Implementation is committed on `codex/zero-redesign` in `/Users/hemangdoshi/.codex/worktrees/zero-redesign/project-zero`; the replayed 173-commit chain is based on the published `main` snapshot and includes the offline display queue fix. The original pre-rebase history is preserved on remote branch `codex/zero-redesign-pre-rebase`. The original checkout at `/Users/hemangdoshi/Developer/project-zero` remains untouched.
...
GitHub `main` is protected: direct pushes are rejected, it requires a pull request and three CI checks, and it disallows merge commits. PR #11 (`codex/promote-electron-redesign` → `main`) published the combined current code tree while preserving all 28 main-only files. Since the squash left the source history unrelated, `codex/zero-redesign` has now been linearly replayed onto that `main` base with all 173 commits retained; the original SHAs remain available on `codex/zero-redesign-pre-rebase`. PR #12 carries that actual commit history into `main` through the protected path. Do not call the history merge complete until PR #12's checks pass, it merges, and the remote ancestry is verified.

The rebased tree passed Electron tests **933/933 across 76 files**, Electron typecheck, and `GOPROXY=off GOSUMDB=off go test ./...`. Only the two handoff documents differ from the currently published main tree; app, runtime and test files match. PR #11's three CI jobs passed. PR #12 must pass the required CI checks before merge. Historical Playwright screenshots and the earlier UI-flow verification are documented in snapshot.md; three screenshots remain untracked and excluded from publication.

The earlier Electron preview has exited. Its unpacked app remains at `apps/desktop/.runtime/desktop-build/mac-arm64/Zero Desktop.app`; it was not installed. The isolated dev profile remains under `~/Library/Application Support/ProjectZero/dev-electron`. No production daemon was started.

## Results

- **Zero Bot:** consecutive reasoning is collapsed; command details use a terminal-style panel; inspector metadata is demoted; thread usage is provider-bound; project/folder groups are collapsible; a draft does not allocate provider identity until accepted send. The actual Playwright send ran against the selected Codex conversation through Airlock, completed two read-only checks and returned a response. A synthetic credential-like draft triggered an Airlock hold and was cancelled before dispatch.
- **OpenCode:** saved local transcript and usage parsing are readable. The host did not advertise a live ACP session/model, so there was no OpenCode live send.
- **Desk:** the page shows the locally observed listening session and history when supplied. A no-daemon preview correctly reports Spotify unavailable and does not claim a connected display. Playlist action remains disabled pending Spotify client/account authorization.
- **Skills Lab:** proposals are opt-in, deterministic and reviewed before installation; learning was left disabled in the isolated profile, and no learned skill was installed. Playwright confirmed the packaged icon asset loads; verified exact identities use local official artwork and unverified identities remain neutral.
- **Runtime:** main-process history is read-only macOS sampling via system counters/commands (including `ps`, `vm_stat`, `sysctl`, `netstat`, `ioreg` and `os.cpus()`), not polling the Activity Monitor app. Playwright verified recent history, CPU/memory/disk/network/GPU chart families, machine-vs-daemon state, and supported CPU/memory process rows. Disk and network per-process ranking are explicitly unavailable. Display session updates do not enqueue new work for nodes beyond the 90-second offline lease.
- **Theme/wallpaper:** dark mode was visibly verified, light/dark selection was toggled through Settings and restored to dark. Wallpaper managed copies and validation are covered by tests; a native file-picker/source-removal/restart walkthrough remains unverified.
- **Flight Recorder:** retained as a bounded-memory inspection of recent event/audit rows with noise filtering. It is useful for recent diagnostics, but it is not a durable event history or an action queue.

## Remaining gates

The Zero daemon/socket is absent in the isolated profile, so registered Projects remain unavailable. This is separate from the working Codex app-server bridge and verified-cwd dispatch. OpenCode local saved data was readable, but live ACP sending was unavailable. No Spotify OAuth/client registration, account connection, playlist write, signed install, production migration or ESP32 physical acceptance was performed. Do not report these as fixed or tested.

To enable playlist creation, register a Spotify client and explicitly authorize the account connection; then implement/test the consented read/write flow. To validate the physical Desk display, use the installation/recovery runbooks and a connected device. For wallpaper UI restart acceptance, exercise the native picker and verify the managed file survives source deletion without touching production preferences.
