...
Updated: **2026-09-22 19:26 IST**. Start with [AGENTS.md](AGENTS.md) and [snapshot.md](snapshot.md). This document supplies durable context; the snapshot supplies the newest checkout/installation/test state. Every agent must maintain both. Pending items are not permission to resume a task the user stopped.

## September 22 follow-up repair handoff

The first follow-up patch transferred as `b9d0a12`, passed full worktree tests, was rebuilt and launched. Live Electron checks confirmed monitor/ESP32 focus. A second red-green patch in the writable clone shortens a monitor stand neck that physically protruded into the panel; transfer/rebuild it next. The missing daemon, broken Codex CLI shims and unimplemented Spotify OAuth remain separate blockers; do not claim full compatibility or cross-device playback.
