...
private final class ClientFixture: @unchecked Sendable {
    ...
    private var fetchContinuation: CheckedContinuation<CockpitSnapshot, Error>?
    ...
    func fetch() async throws -> CockpitSnapshot {
        try await withCheckedThrowingContinuation { result in lock.withLock { fetches += 1; fetchContinuation = result } }
    }
    func releaseFetch() {
        let result = lock.withLock { let value = fetchContinuation; fetchContinuation = nil; return value }
        result?.resume(with: Result { try CockpitSnapshot.decode(data) })
    }
