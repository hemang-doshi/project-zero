...
describe('OpenCode session export', () => {
      ...
      messages: [
        { info: { id: 'm1', role: 'user' }, parts: [{ id: 'p1', type: 'text', text: 'Fix it' }] },
        {
          ...
          parts: [
            ...
            { id: 'p3', type: 'tool', tool: 'bash', state: { status: 'completed', input: { command: 'pwd' }, output: '/tmp', metadata: { exitCode: 0 } } },
            { id: 'p4', type: 'tool', tool: 'read', state: { status: 'completed', input: { filePath: 'a.ts' }, output: 'content' } },
            { id: 'p5', type: 'text', text: 'Done' }
          ]
        }
      ]
    })
    expect(result.items.map((item) => item.kind)).toEqual(['message', 'thinking', 'exec', 'tool', 'message'])
    expect(result.items[2]).toMatchObject({ kind: 'exec', command: 'pwd', output: '/tmp' })
    expect(result.items[3]).toMatchObject({ kind: 'tool', tool: 'read', detail: '{"filePath":"a.ts"}' })
  })
  ...
  it('rejects malformed export and bounds visible history', () => {
    ...
    const messages = Array.from({ length: 405 }, (_, i) => ({
      info: { id: `m${i}`, role: 'user' }, parts: [{ type: 'text', text: `${i}` }]
    }))
